...
QString BibleOnlineSource::chapterUrl(Translation* translation, const QString &bookCode, int chapterNo) const
{
    int no = bookCodes(translation).indexOf(bookCode);
    if (no == -1)
        return "<illegal url>";
...
ChapterRequest* BibleOnlineSource::requestChapter(Translation* translation, QNetworkAccessManager *nam, const QString& bookCode, int chapterNo)
{
