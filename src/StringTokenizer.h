...
{
    public:
        typedef void (*token_callback)(const QString& token, int start_offset, int end_offset, int position);


        inline static const bool is_spacing(QChar c)
        {
        ...
        static void tokenize(const QString& text, token_callback callback)
        {
            ...
            while (data[text_pos] != 0)
            {
                ...
                callback(token, start_offset, text_pos, position);
                position++;
            }
        }
};
