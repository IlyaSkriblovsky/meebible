...
int main()
{
    ...
    sqlite3_exec(db, "INSERT INTO text (content) VALUES ('<div>Öö Jêsūs ßß ss')", 0, 0, 0);
    sqlite3_exec(db, "SELECT * FROM text WHERE content MATCH 'UPPHoJD'", callback, 0, 0);

    // const char *transCodes[] = { /*"ara",*/ "bbg", "blv", "esv", "frs", "kjv", "niv", "pal", "rus", "rvr", "svf" };

    // sqlite3_exec(db, "CREATE VIRTUAL TABLE text USING fts4 (transCode, langCode, bookCode, chapterNo, content, tokenize=ilya)", 0, 0, 0);

    // for (int i = 0; i < sizeof(transCodes) / sizeof(transCodes[0]); i++)
    // {
    //     char *err;
    //     printf("%s\n", transCodes[i]);
    //     char buf[1024];
    ...
    //     snprintf(buf, 1024, "DETACH '%s'", transCodes[i], transCodes[i]);
    //     sqlite3_exec(db, buf, 0, 0, &err);
